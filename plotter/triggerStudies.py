...
def efficiency(dataset, name, savename="", binning=None, binningName="", additional=[]):
    ...
    # axis=TH1F("","",len(binning),binning[0],binning[-1])
    # axis.SetStats(0)
    # axis.Draw("axis")
    ...
    if additional:
        ...
        if effAdd.UsesWeights():
            effAdd.SetStatisticOption(ROOT.TEfficiency.kFNormal)
    ...
    if cutValue or True:
        ...
        eLabel = ROOT.TLatex(0.47, .37, "#varepsilon_{{Data}} = {:.1f}^{{#plus{:.1f}}}_{{#minus{:.1f}}}%".format(
            100 * e, 100 * (e_up - e), 100 * (e - e_dn)))
        ...
        eLabel.SetNDC()
        eLabel.SetTextSize(0.03)
        eLabel.Draw()
        if additional:
            if passedAdd <= totalAdd:
                confAdd = ROOT.TEfficiency().GetConfidenceLevel()
                e_upAdd = ROOT.TEfficiency.ClopperPearson(
                    totalAdd, passedAdd, confAdd, True)
                e_dnAdd = ROOT.TEfficiency.ClopperPearson(
                    totalAdd, passedAdd, confAdd, False)
            else:
                passedAdd, epassedAdd = aux.integralAndError(
                    h_pasAdd, binAdd, -1)
                totalAdd, etotalAdd = aux.integralAndError(
                    h_pasAdd, binAdd, -1)
                eeAdd = eAdd * \
                    math.sqrt((epassedAdd / passedAdd) **
                              2 + (etotalAdd / totalAdd)**2)
                e_upAdd = eAdd + eeAdd
                e_dnAdd = eAdd - eeAdd
        ...
        effAddSystHist = aux.getSysHisto(tempH, 0.03)
        effAddSyst = aux.getSysHistoGraphTrigger(tempH, 0.03)
        #effAddSyst = aux.getSysHisto(grAdd, 0.03)
        aux.drawOpt(effAddSyst, "sysUncGraph")
        ...
        # for i in range(effAddSyst.GetN()):
        #print effAddSyst.GetY()[i]
        effAddSyst.Draw("same 2")
        #effAddSyst.Draw("same p")
        # effAddSyst.Draw("SAME")
...
#
    ...
    #eff.GetXaxis().SetRangeUser(0., 100.)
    #effAdd.GetXaxis().SetRangeUser(0., 100.)
    ...
    # dataHist=eff.GetPassedHistogram().Clone()
    dataHist = h_pas.Clone()
    dataHist.SetTitle("")
    dataHist.SetMarkerColor(ROOT.kBlack)
    # mcHist=effAdd.GetPassedHistogram().Clone()
    if additional:
        mcHist = h_pasAdd.Clone()
        mcHist.SetTitle("")
        mcHist.SetMarkerColor(ROOT.kBlack)

    # dataGr=eff.Clone()
    # mcGr=effAdd.Clone()

        for bin in range(-1, dataHist.GetNbinsX() + 1):
            dataHist.SetBinContent(bin, eff.GetEfficiency(bin))
            dataHist.SetBinError(bin, max(eff.GetEfficiencyErrorUp(
                bin), eff.GetEfficiencyErrorLow(bin)))
            #print dataHist.GetBinLowEdge(bin)
            #print eff.GetEfficiencyErrorUp(bin),eff.GetEfficiencyErrorLow(bin)
            mcHist.SetBinContent(bin, effAdd.GetEfficiency(bin))
            mcHist.SetBinError(bin, max(effAdd.GetEfficiencyErrorUp(
                bin), effAdd.GetEfficiencyErrorLow(bin)))
            #print effAdd.GetEfficiencyErrorUp(bin),effAdd.GetEfficiencyErrorLow(bin)
            #print mcHist.GetBinLowEdge(bin)

        mcHist.Scale(e / eAdd)
        rSyst = effAddSystHist.Clone()
        rSyst.Scale(e / eAdd)
    ...
    aux.save("efficiency_" + savename + name + binningName,
             log=False, folder="triggerStudies/")
...
def efficiencyAllMC(dataset, name, savename="", binning=None, binningName="", additional=[]):
    ...
    arAdd = [effAdd_zgamma, effAdd_ttgamma, effAdd_zz, effAdd_wwgamma, effAdd_wzgamma, effAdd_DYjetsNLO,
             effAdd_wjets, effAdd_tt, effAdd_zz4l, effAdd_wz, effAdd_ww, effAdd_singletop, effAdd_wgamma]

    for i in range(len(arAdd)):
        if arAdd[i].UsesWeights():
            arAdd[i].SetStatisticOption(ROOT.TEfficiency.kFNormal)
    ...
    arAddPas = [zgammaPas, ttgammaPas, zzPas, wwgammaPas, wzgammaPas, DYjetsNLOPas,
                wjetsPas, ttPas, zz4lPas, wzPas, wwPas, singletopPas, wgammaPas]
    arAddTot = [zgammaTot, ttgammaTot, zzTot, wwgammaTot, wzgammaTot, DYjetsNLOTot,
                wjetsTot, ttTot, zz4lTot, wzTot, wwTot, singletopTot, wgammaTot]
    ...
    if name.endswith("_ps"):
        ...
        xAr = [x_zgamma, x_ttgamma, x_zz, x_wwgamma, x_wzgamma, x_DYjetsNLO,
               x_wjets, x_tt, x_zz4l, x_wz, x_ww, x_singletop, x_wgamma]
        yAr = [y_zgamma, y_ttgamma, y_zz, y_wwgamma, y_wzgamma, y_DYjetsNLO,
               y_wjets, y_tt, y_zz4l, y_wz, y_ww, y_singletop, y_wgamma]
        effAr = [eff_zgamma, eff_ttgamma, eff_zz, eff_wwgamma, eff_wzgamma, eff_DYjetsNLO,
                 eff_wjets, eff_tt, eff_zz4l, eff_wz, eff_ww, eff_singletop, eff_wgamma]
    ...
    if cutValue or True:
        ...
        if e_upSyst > 1.:
            e_upSyst = 1.
        eLabel = ROOT.TLatex(0.47, .17, "#varepsilon_{{Data}} = {:.1f}^{{#plus{:.1f}}}_{{#minus{:.1f}}}%".format(
            100 * e, 100 * (e_up - e), 100 * (e - e_dn)))
        eLabel.SetNDC()
        eLabel.Draw()
        # if additional:
...
def main():
    ...
    allMC = zgamma + ttgamma + zz + wwgamma + wzgamma + \
        DYjetsNLO + wjets + tt + zz4l + wz + ww + singletop + wgamma
    #allMC = zgamma+ttgamma+zz+wwgamma+wzgamma+DYjetsNLO+wjets+zz4l+wz+ww+singletop+wgamma
    # variables=["eta1","pt1","n_jets","n_vtx","phi1","m_ll","ht","n_photons"]
    variables = ["pt1", "pt2", "ht", "met"]
    # variables = ["pt2"]
    ...
    for group in groups:
        for variable in variables:
            ...
            #                group + "_EE", binning=binnings[variable], additional=[allMC])
            # d = efficiency(dataMET, group + "/MM/" + variable, "dataMET_" +
            #                group + "_MM", binning=binnings[variable], additional=[allMC])
            # d = efficiency(dataMET, group + "/EM/" + variable, "dataMET_" +
            #                group + "_EM", binning=binnings[variable], additional=[allMC])
            #
            # ee_data, ee_mc = efficiency(dataMET, group + "_ptcuts/EE/" + variable, "dataMET_" +
            #                             group + "_ptcuts_EE", binning=binnings[variable], additional=[allMC])
            # mm_data, mm_mc = efficiency(dataMET, group + "_ptcuts/MM/" + variable, "dataMET_" +
            #                             group + "_ptcuts_MM", binning=binnings[variable], additional=[allMC])
            # em_data, em_mc = efficiency(dataMET, group + "_ptcuts/EM/" + variable, "dataMET_" +
            #                             group + "_ptcuts_EM", binning=binnings[variable], additional=[allMC])

            # d=efficiency(dataMET,group+"/EE/"+variable,"dataHT_"+group+"_EE_MET",binning=binnings[variable])
            # d=efficiency(dataMET,group+"/MM/"+variable,"dataHT_"+group+"_MM_MET",binning=binnings[variable])
            # d=efficiency(dataMET,group+"/EM/"+variable,"dataHT_"+group+"_EM_MET",binning=binnings[variable])

            # es=efficiency(dataMET,group+"_ptcuts/EE/"+variable,"dataHT_"+group+"_ptcuts_EE_MET",binning=binnings[variable])
            ...
            # q=efficiency(allMC,group+"_ptcuts/EM/"+variable,"dataHT_"+group+"_ptcuts_EM_PURE",binning=binnings[variable])
            ...
            # efficiency(dataHt,group+"_ptcuts/EE/"+variable,"dataHT_"+group+"_ptcuts_EE",binning=binnings[variable])
            # efficiency(dataHt,group+"_ptcuts/MM/"+variable,"dataHT_"+group+"_ptcuts_MM",binning=binnings[variable])
            # efficiency(dataHt,group+"_ptcuts/EM/"+variable,"dataHT_"+group+"_ptcuts_EM",binning=binnings[variable])
            ...
            # savepkl={}
            # savepkl["ee"]={}
            # savepkl["ee"]["data"]=ee_data
            # savepkl["ee"]["mc"]=ee_mc
            # savepkl["ee"]["factor"]=ee_data/ee_mc
            # savepkl["em"]={}
            # savepkl["em"]["data"]=em_data
            # savepkl["em"]["mc"]=em_mc
            # savepkl["em"]["factor"]=em_data/em_mc
            # savepkl["mm"]={}
            # savepkl["mm"]["data"]=mm_data
            # savepkl["mm"]["mc"]=mm_mc
            # savepkl["mm"]["factor"]=mm_data/mm_mc
            #
            #
            # pkl.dump( savepkl, open( "triggerStudies/triggerFactors.pkl", "wb" ) )

            newBinnings = binnings.copy()
            newBinnings["pt1"] = frange(0., 300, 5.)
            newBinnings["pt2"] = frange(0., 100, 10.)
            ...
            # efficiencyAllMC(allMC,group+"_ptcuts/MM/"+variable,"test_"+group+"_MM",binning=newBinnings[variable],additional=[zgamma,ttgamma,zz,wwgamma,wzgamma,DYjetsNLO,wjets,tt,zz4l,wz,ww,singletop,wgamma])
            # efficiencyAllMC(allMC,group+"_ptcuts/EM/"+variable,"test_"+group+"_EM",binning=newBinnings[variable],additional=[zgamma,ttgamma,zz,wwgamma,wzgamma,DYjetsNLO,wjets,tt,zz4l,wz,ww,singletop,wgamma])
            # efficiencyAllMC(allMC,group+"_ptcuts/EE/"+variable,"test_"+group+"_EE",binning=newBinnings[variable],additional=[zgamma,ttgamma,zz,wwgamma,wzgamma,DYjetsNLO,wjets,tt,zz4l,wz,ww,singletop,wgamma])
