...
def drawVR(sampleNames, name, binning=None, binningName="", xTitle=None, yTitle=None, sfZZ=[], sfDY=[], sfTT=[], sfWZ=[]):
    ...
    signal1 = aux.stdHistWithWeights(
        ...
        gmsb_415_355, name, ["nISR", "topPt", "ewk"], binning)
    signal3 = aux.stdHistWithWeights(
        tching_400, name, ["nISR", "topPt", "ewk"], binning)
    signal4 = aux.stdHistWithWeights(
        tching_600, name, ["nISR", "topPt", "ewk"], binning)
    signal5 = aux.stdHistWithWeights(
        t5bbbbzg_1500_400, name, ["nISR", "topPt", "ewk"], binning)
    signal6 = aux.stdHistWithWeights(
        t5bbbbzg_1500_1400, name, ["nISR", "topPt", "ewk"], binning)
    ...
    for h in signal3, signal4:
        aux.drawOpt(h, "signal")
    for h in signal5, signal6:
        aux.drawOpt(h, "signal")
    signal1.SetLineColor(ROOT.kBlue + 3)
    signal2.SetLineColor(ROOT.kBlue + 3)
    signal2.SetLineStyle(2)
    signal3.SetLineStyle(1)
    signal4.SetLineStyle(2)
    signal5.SetLineStyle(1)
    signal6.SetLineStyle(2)

    signal3.SetLineColor(ROOT.kRed - 4)
    signal4.SetLineColor(ROOT.kRed - 4)
    signal5.SetLineColor(ROOT.kCyan)
    signal6.SetLineColor(ROOT.kCyan)
...
binnings_["mt2"] = frange(80, 250, 10)
binnings_["met"] = frange(0, 440, 20)
