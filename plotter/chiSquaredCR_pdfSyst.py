...
def drawSameHistogram( sampleNames, name, bkg=[], additional=[], binning=None, binningName="", scaleToData=True, xTitle=None, yTitle=None,toScale=[],SF=1. ):
    ...
    for d in bkg[-1::-1]:
        #print d.names
        if('ST_tW_antitop_5f_NoFullyHadronicDecays_ext') in d.names or ('ST_tW_top_5f_NoFullyHadronicDecays_ext') in d.names:
            #print "-----------"
            h = d.getHist( name.replace("/"+intPDF+"/","/nom/") )
            #print folder
            avgTopPtWeightHisto = d.getHist(folder.replace("/"+intPDF+"","/nom")+"/weight_topPt")
            avgNIsrWeightHisto = d.getHist(folder.replace("/"+intPDF+"","/nom")+"/weight_nISR")
            avgEWKinoWeightHisto = d.getHist(folder.replace("/"+intPDF+"","/nom")+"/weight_EWKinoPairPt")
            avgleptonWeightHisto = d.getHist(folder.replace("/"+intPDF+"","/nom")+"/weight_leptonPairPt")
            avgPDFWeightHisto = d.getHist(folder.replace("/"+intPDF+"","/nom")+"/weight_PDF")
        else:
            h = d.getHist( name )
            ...
            avgPDFWeightHisto = d.getHist(folder+"/weight_PDF")
        ...
        if avgleptonWeightHisto: 
            ...
            avgleptonWeight=1.
        if avgPDFWeightHisto: 
            avgPDFWeight = avgPDFWeightHisto.GetMean()
        else:
            avgPDFWeight=1.
...
def drawDY(binningToUse_=CR_DY.binnings.copy(),addName=""):
    ...
    for group in groups:
        ...
        for variable in variables:
            ...
            for iPDF in range(9):
                saveValuesEE[group][variable][iPDF]={}
                saveValuesMM[group][variable][iPDF]={}
                saveValuesLL[group][variable][iPDF]={}
                chiValuesEE=[]
                chiValuesMM=[]
                chiValuesLL=[]
                for SF in scaleFactors:
                    tempEE=(drawSameHistogram("EE",group+"/EE/"+additionalFolder.replace("/",str(iPDF)+"/")+variable, bkgs, additional=[dataDoubleEG],binning=binningToUse_[variable],xTitle=labels[variable][0],toScale=[DYjetsNLO,zgamma],SF=SF))
                    tempMM=(drawSameHistogram("MM",group+"/MM/"+additionalFolder.replace("/",str(iPDF)+"/")+variable, bkgs, additional=[dataDoubleMuon],binning=binningToUse_[variable],xTitle=labels[variable][0],toScale=[DYjetsNLO,zgamma],SF=SF))
                    tempLL=(drawSameHistogram("LL",group+"/LL/"+additionalFolder.replace("/",str(iPDF)+"/")+variable, bkgs, additional=[dataDoubleSF],binning=binningToUse_[variable],xTitle=labels[variable][0],toScale=[DYjetsNLO,zgamma],SF=SF))
                ...
                l.SetNDC()
                l.Draw()
                l2 = ROOT.TLatex( 0.47, .65, "#scale[0.66]{#font[52]{#alpha = %.3f^{#plus%.3f}_{#minus%.3f}}}"%(sfResultLL,upLL,downLL))
                l2.SetNDC()
                l2.Draw()
                l3 = ROOT.TLatex( 0.47, .6, "#scale[0.66]{#font[52]{#chi}^{2} = %.2f / %.0f}"%(chiResultLL,tempLL[1]))
                l3.SetNDC()
                l3.Draw()
                l4 = ROOT.TLatex( 0.47, .7, "#scale[0.66]{#font[52]{DY Control Region ee+#mu#mu}}")
                l4.SetNDC()
                l4.Draw()
                lum = ROOT.TLatex( .62, .95, "%.1f fb^{-1} (%s TeV)"%(aux.intLumi/1000., aux.Label.cmsEnergy) )
                lum.SetNDC()
                ...
                c.Update()
                c.SaveAs('plots_CR/chi/DY_LL'+'_'+variable+addName+str(iPDF)+'.pdf')
                c.SaveAs('plots_CR/chi/DY_LL'+'_'+variable+addName+str(iPDF)+'.root')
                ...
                erDownEE=downEE
                erDownMM=downMM
                saveValuesEE[group][variable][iPDF]["value"]=bestValueEE
                saveValuesEE[group][variable][iPDF]["erUp"]=erUpEE
                saveValuesEE[group][variable][iPDF]["erDown"]=erDownEE
                saveValuesMM[group][variable][iPDF]["value"]=bestValueMM
                saveValuesMM[group][variable][iPDF]["erUp"]=erUpMM
                saveValuesMM[group][variable][iPDF]["erDown"]=erDownMM
                saveValuesLL[group][variable][iPDF]["value"]=bestValueLL
                saveValuesLL[group][variable][iPDF]["erUp"]=erUpLL
                saveValuesLL[group][variable][iPDF]["erDown"]=erDownLL
    pkl.dump( saveValuesEE, open( "plots_CR/chi/DY_chi_EE"+addName+".pkl", "wb" ) )
    pkl.dump( saveValuesLL, open( "plots_CR/chi/DY_chi_LL"+addName+".pkl", "wb" ) )
...
def drawZZ(binningToUse_=CR_ZZ.binnings.copy(),addName=""):
    ...
    #variables=["eta1","eta2","pt1","pt2","pt_g1","m_ll","phi1","phi2","n_jets","ht"]
    variables=["eta1","eta2","pt1","pt2","phi1","phi2","n_jets","ht","m_ll","m_ll2","met"]
    #variables=["pt1"]
    bkgs=[DYjetsNLO,zgamma,tt,ttgamma,wwgamma,wzgamma,zz,wjets,wgamma,singletop,zz4l,wz,ww]
    groups=["CRZZ"]
    scaleFactors=frange(1.2,1.61,0.01)
    ...
    for group in groups:
        ...
        for variable in variables:
            ...
            for iPDF in range(9):
                saveValues[group][variable][iPDF]={}
                chiValues=[]
                for SF in scaleFactors:
                    tempLL=(drawSameHistogram("LL",group+"/LL/"+additionalFolder.replace("/",str(iPDF)+"/")+variable, bkgs, additional=[dataLL],binning=binningToUse_[variable],xTitle=labels[variable][0],toScale=[zz4l],SF=SF))
                ...
                l2.SetNDC()
                l2.Draw()
                l3 = ROOT.TLatex( 0.47, .6, "#scale[0.66]{#font[52]{#chi}^{2} = %.2f / %.0f}"%(chiResult,tempLL[1]))
                l3.SetNDC()
                l3.Draw()
                l4 = ROOT.TLatex( 0.47, .7, "#scale[0.66]{#font[52]{ZZ Control Region}}")
                l4.SetNDC()
                l4.Draw()
                lum = ROOT.TLatex( .62, .95, "%.1f fb^{-1} (%s TeV)"%(aux.intLumi/1000., aux.Label.cmsEnergy) )
                lum.SetNDC()
                ...
                c.Update()
                c.SaveAs('plots_CR/chi/ZZ'+'_'+variable+addName+str(iPDF)+'.pdf')
                c.SaveAs('plots_CR/chi/ZZ'+'_'+variable+addName+str(iPDF)+'.root')
                
                bestValue=sfResult
                erUp=up
                erDown=down
                saveValues[group][variable][iPDF]["value"]=bestValue
                saveValues[group][variable][iPDF]["erUp"]=erUp
                saveValues[group][variable][iPDF]["erDown"]=erDown
...
def drawWZ(binningToUse_=CR_WZ.binnings.copy(),addName=""):
    ...
    #variables=["eta1","eta2","pt1","pt2","met","m_ll","phi1","phi2","n_jets","ht"]
    #variables=["eta1","eta2","pt1","pt2","pt_g1","m_ll","phi1","phi2","n_jets","ht"]
    variables=["eta1","eta2","pt1","pt2","phi1","phi2","n_jets","ht","m_ll","met"]
    #variables=["pt1"]
    bkgs=[DYjetsNLO,zgamma,tt,ttgamma,wwgamma,wzgamma,zz,wjets,wgamma,singletop,zz4l,wz,ww]
    groups=["CRWZ"]
    scaleFactors=frange(1.1,1.305,0.01)
    ...
    for group in groups:
        ...
        for variable in variables:
            ...
            for iPDF in range(9):
                ...
                chiValues=[]
                for SF in scaleFactors:
                    tempLL=(drawSameHistogram("LL",group+"/LL/"+additionalFolder.replace("/",str(iPDF)+"/")+variable, bkgs, additional=[dataLL],binning=binningToUse_[variable],xTitle=labels[variable][0],toScale=[wz],SF=SF))
                    chiValues.append(tempLL[0])
                gr = TGraph();
                ...
                l3.SetNDC()
                l3.Draw()
                l4 = ROOT.TLatex( 0.47, .7, "#scale[0.66]{#font[52]{WZ Control Region}}")
                l4.SetNDC()
                l4.Draw()
                lum = ROOT.TLatex( .62, .95, "%.1f fb^{-1} (%s TeV)"%(aux.intLumi/1000., aux.Label.cmsEnergy) )
                lum.SetNDC()
                ...
                c.Update()
                c.SaveAs('plots_CR/chi/WZ'+'_'+variable+addName+str(iPDF)+'.pdf')
                c.SaveAs('plots_CR/chi/WZ'+'_'+variable+addName+str(iPDF)+'.root')
...
def drawTT(binningToUse_=CR_tt.binnings.copy(),addName=""):
    ...
    scaleFactors=frange(0.8,1.155,0.01)
    ...
    for group in groups:
        ...
        for variable in variables:
            ...
            for iPDF in range(9):
                ...
                chiValues=[]
                for SF in scaleFactors:
                    tempEM=(drawSameHistogram("EM",group+"/EM/"+additionalFolder.replace("/",str(iPDF)+"/")+variable, bkgs, additional=[dataLL],binning=binningToUse_[variable],xTitle=labels[variable][0],toScale=[tt,ttgamma],SF=SF))
                ...
                c.Update()
                c.SaveAs('plots_CR/chi/TT'+'_'+variable+addName+str(iPDF)+'.pdf')
                c.SaveAs('plots_CR/chi/TT'+'_'+variable+addName+str(iPDF)+'.root')
...
def main():
    ...
    drawDY(binningToUse_=nominalBinningDY,addName="_pdf")
    ...
    #drawZZ(binningToUse_=nominalBinningZZ,addName="_pdf")
    ...
    #drawWZ(binningToUse_=nominalBinningWZ,addName="_pdf")
