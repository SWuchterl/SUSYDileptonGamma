...
class smsPlotXSEC(smsPlotABS):
    ...
    def __init__(self, modelname, histo, obsLimits, expLimits, energy, lumi, preliminary, label):
        self.LABEL = label
        self.standardDef(modelname, histo, obsLimits,
                         expLimits, energy, lumi, preliminary)
        # canvas for the plot
    ...
    def setStyleCOLZ(self):
        ...
        self.histo.GetZaxis().SetLabelSize(0.033)
        self.histo.GetZaxis().SetTitleSize(0.035)
        self.histo.Scale(1000)  # from pb to fb
        self.histo.SetMinimum(
            self.model.Zmin * 1000 if self.model.Zmin > 0 else self.histo.GetMinimum())
        self.histo.SetMaximum(
            self.model.Zmax * 1000 if self.model.Zmax > 0 else self.histo.GetMaximum())

        self.histo.GetZaxis().SetMoreLogLabels(True)
        ...
        rt.TColor.CreateGradientColorTable(
    ...
    def DrawPaletteLabel(self):
        textCOLZ = rt.TLatex(
            0.99, 0.15, "95% CL upper limit on cross section (fb)")
        textCOLZ.SetNDC()
